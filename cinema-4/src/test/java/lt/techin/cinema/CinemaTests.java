...
public class CinemaTests {
    ...
    @Test
    public void testStatsEndpointWithWrongPassword() throws Exception {
        mockMvc.perform(get("/stats"))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.error").value(WRONG_PASSWORD_MESSAGE));
        mockMvc.perform(get("/stats").param("password", "wrong_password"))
                .andExpect(status().isUnauthorized());
    }

    @Test
    public void testStatsWithCorrectPassword() throws Exception {
        mockMvc.perform(get("/stats").param("password", "super_secret"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.income").value(0))
                .andExpect(jsonPath("$.available").value(81))
                .andExpect(jsonPath("$.purchased").value(0))
        ;
        Map<String, Object> purchaseRequest = Map.of("row", 5, "column", 6);
        String purchaseResponseBody = mockMvc.perform(post("/purchase")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(purchaseRequest)))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/stats").param("password", "super_secret"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.income").value(8))
